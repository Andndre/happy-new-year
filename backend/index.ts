...
type Message = {type: "launch", name: string} | {type: "join", name: string, new: boolean} | {type: "connected"} | {type: "leave", name: string} | {type: 'error', error: string};
    ...
    case "launch":
      players.forEach((player, pName) => {
        if (message.name !== pName && player.active) {
          player.ws.send(JSON.stringify({ type: "launch", name: message.name }));
        }
      });
      break;
    case 'join':
      if (players.has(message.name) && message.new) {
        ws.send(JSON.stringify({type: 'error', error: 'name_already_exist'} as Message));
        break;
      } else if (!message.new) {
        players.set(message.name, {active: true, ws});
        ws.send(JSON.stringify({type: 'launch', name: message.name} as Message))
      }
      players.forEach((player, name) => {
        if (message.name !== name && player.active) {
